...
const fetchData = async () => {
  ...
  if (access_token) {
    ...
    try {
      const response = await axios.get(`${url}/tnlcm/library/components/upf_p4_sw`, {
        headers: {
      ...
      alert(`Error fetching data for upf_p4_sw:`, error);
      return null;
    }
  }
  return null;
...
const UpfP4Sw = ({ id, removeComponent, onChange, list, whenError, defaultValues, name }) => {
  ...
  useEffect(() => {
      ...
      const networks = Array.isArray(formValues["one_upf_p4_sw_networks"])
        ? formValues["one_upf_p4_sw_networks"]
        : [];
      ...
      if (validNetworks.length !== networks.length) {
        setFormValues((prevState) => ({
          ...prevState,
          "one_upf_p4_sw_networks": validNetworks,  // Update the form values
        }));
    
        // Call onChange to update the state in the parent component
        onChange(id, "one_upf_p4_sw_networks", validNetworks);
      }
  ...
  const handleChange = (event) => {
    ...
    if (name === "one_upf_p4_sw_open5gs_amf_ngap_addr" || name === "one_upf_p4_sw_controller_upf_ipv4_n3"
        || name === "one_upf_p4_sw_controller_upf_ipv4_n6" || name === "one_upf_p4_sw_controller_enb_ipv4_n3"
        || name === "one_upf_p4_sw_controller_dn_ipv4_n6"
     ) { // Special validation for being sure that only IPs are allowed
      ...
      if (name === "one_upf_p4_sw_open5gs_control_mcc") {
        if (!isValidMCC(value)) {
          setErrorMessages((prevState) => ({
            ...prevState,
          ...
          ((value ?? "").replace(/"/g, "").length +
            (formValues.one_upf_p4_sw_open5gs_control_mnc ?? "").replace(/"/g, "").length +
            (formValues.one_upf_p4_sw_open5gs_control_msin ?? "").replace(/"/g, "").length) !== 15
        ) {
          setErrorMessages((prevState) => ({
            ...prevState,
      ...
      if (name === "one_open5gs_vm_mnc") {
        if (!isValidMNC(value)) {
          ...
          ((formValues.one_upf_p4_sw_open5gs_control_mcc ?? "").replace(/"/g, "").length +
            (value ?? "").replace(/"/g, "").length +
            (formValues.one_upf_p4_sw_open5gs_control_mnc ?? "").replace(/"/g, "").length) !== 15
        ) {
          setErrorMessages((prevState) => ({
            ...prevState,
            [name]: `MCC + MNC + MSIN must add to exactly 15 digits`,
    ...
    return (
      <div className="bg-gray-100 p-6">
        <header className="bg-blue-500 text-white text-center p-4 rounded-md shadow-md">
          ...
          <h1 className="text-3xl font-bold">UPF_P4_SW Added</h1>
          <p className="mt-2">The UPF_P4_SW component has been added successfully.</p>
        </header>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-100 p-6">
      ...
      <header className="bg-blue-500 text-white text-center p-4 rounded-md shadow-md">
        ...
        <h1 className="text-3xl font-bold">UPF_P4_SW Configuration</h1>
      ...
      <div className="mt-8 bg-white shadow-md rounded-lg p-6">
        <form>
          ...
          {Object.keys(data).map((key) => {
            ...
            if (key === "one_upf_p4_sw_networks") {
              return (
                <div key={key} className="mb-4">
                  <label className="block text-gray-7S00 font-semibold mb-2">
