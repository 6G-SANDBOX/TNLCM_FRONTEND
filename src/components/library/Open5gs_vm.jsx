...
const fetchData = async () => {
  ...
  if (access_token) {
    ...
    try {
      const response = await axios.get(`${url}/tnlcm/library/components/open5gs_vm`, {
        headers: {
      ...
      alert(`Error fetching data for open5gs_vm:`, error);
      return null;
    }
  }
  return null;
...
const Open5gsVM = ({ id, removeComponent, onChange, list1, list2,  whenError, defaultValues, name }) => {
  ...
  useEffect(() => {
      ...
      const networks1 = Array.isArray(formValues["one_open5gs_vm_networks"])
        ? formValues["one_open5gs_vm_networks"]
        : [];
      ...
      if (validNetworks1.length !== networks1.length) {
        setFormValues((prevState) => ({
          ...prevState,
          "one_open5gs_vm_networks": validNetworks1,  // Update the selected networks state
        }));
    
        // Call onChange to update the state in the parent component
        onChange(id, "one_open5gs_vm_networks", validNetworks1);
      }
  
      const networks2 = Array.isArray(formValues["one_open5gs_vm_internal_vnet"])
        ? formValues["one_open5gs_vm_internal_vnet"]
        : [];
      const validNetworks2 =networks2.filter((network) => list2.includes(network));
      if (validNetworks2.length !== networks2.length) {
        setFormValues((prevState) => ({
          ...prevState,
          "one_open5gs_vm_internal_vnet": validNetworks2,  // Update the selected networks state
        }));
    
        // Call onChange to update the state in the parent component
        onChange(id, "one_open5gs_vm_internal_vnet", validNetworks2);
      }
  ...
  const handleChange = (event) => {
    ...
    if (name === "one_open5gs_vm_amf_ip" || name === "one_open5gs_vm_upf_ip") {
    ...
    if (name === "one_open5gs_vm_mcc") {
      if (!isValidMCC(value)) {
        ...
        ((value ?? "").replace(/"/g, "").length +
          (formValues.one_open5gs_vm_mnc ?? "").replace(/"/g, "").length +
          (formValues.one_open5gs_vm_msin ?? "").replace(/"/g, "").length) !== 15
      ) {
        setErrorMessages((prevState) => ({
          ...prevState,
    ...
    if (name === "one_open5gs_vm_mnc") {
      if (!isValidMNC(value)) {
        ...
        ((formValues.one_open5gs_vm_mcc ?? "").replace(/"/g, "").length +
          (value ?? "").replace(/"/g, "").length +
          (formValues.one_open5gs_vm_msin ?? "").replace(/"/g, "").length) !== 15
      ) {
        setErrorMessages((prevState) => ({
    ...
    if (name === "one_open5gs_vm_s_nssai_sd"){
    ...
    return (
      <div className="bg-gray-100 p-6">
        <header className="bg-blue-500 text-white text-center p-4 rounded-md shadow-md">
          ...
          <h1 className="text-3xl font-bold">OPEN5GS_VM Added</h1>
          <p className="mt-2">The OPEN5Gs_VM component has been added successfully.</p>
        </header>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-100 p-6">
      ...
      <header className="bg-blue-500 text-white text-center p-4 rounded-md shadow-md">
        ...
        <h1 className="text-3xl font-bold">Open5GS_VM Config</h1>
      ...
      <div className="mt-8 bg-white shadow-md rounded-lg p-6">
        <form>
          ...
          {Object.keys(data).map((key) => {
            ...
            if (key === "one_open5gs_vm_networks"){
                return (
              ...
              if(key === "one_open5gs_vm_internal_vnet"){
                return (
                  <div key={key} className="mb-4">
                    <label className="block text-gray-700 font-semibold mb-2">
                    {key.replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase())}:
