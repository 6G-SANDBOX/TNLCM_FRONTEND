...
const fetchData = async () => {
  ...
  if (access_token) {
    ...
    try {
      const response = await axios.get(`${url}/tnlcm/library/components/iswireless_radio`, {
        headers: {
      ...
      alert(`Error fetching data for iswireless_radio:`, error);
      return null;
    }
  }
  return null;
...
const IswirelessRadio = ({ id, removeComponent, onChange, list, whenError, defaultValues, name }) => {
  ...
  useEffect(() => {
    if (prevListRef.current?.length !== list.length) {
      ...
      if (list.length === 0 && formValues['one_iswireless_radio_open5gs'] !== "") {
        onChange(id, 'one_iswireless_radio_open5gs', "");
      }
    }
    ...
    return (
      <div className="bg-gray-100 p-6">
        <header className="bg-blue-500 text-white text-center p-4 rounded-md shadow-md">
          ...
          <h1 className="text-3xl font-bold">ISWIRELESS_RADIO Added</h1>
          <p className="mt-2">The ISWIRELESS_RADIO component has been added successfully.</p>
        </header>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-100 p-6">
      ...
      <header className="bg-blue-500 text-white text-center p-4 rounded-md shadow-md">
        ...
        <h1 className="text-3xl font-bold">ISWIRELESS_RADIO Config</h1>
      ...
      <div className="mt-8 bg-white shadow-md rounded-lg p-6">
        <form>
          ...
          {Object.keys(data).map((key) => {
            ...
            if (key === "one_iswireless_radio_open5gs") {
              return (
                <div key={key} className="mb-4">
                  ...
                  {list.length === 0 || list === ""
                    ? "Create news Open5gs_vm or Open5gs_k8s to be able to select"
                    : field.description
                  }
                </small>
                </div>
...
export default IswirelessRadio;
