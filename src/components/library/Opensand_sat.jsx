...
const OpensandSat = ({ id, removeComponent, onChange, list, whenError, defaultValues, name, request }) => {
  ...
  useEffect(() => {
      ...
      const networks = Array.isArray(formValues["one_opensand_sat_networks"])
        ? formValues["one_opensand_sat_networks"]
        : [];
      ...
      if (validNetworks.length !== networks.length) {
        setFormValues((prevState) => ({
          ...prevState,
          "one_opensand_sat_networks": validNetworks,  // Update the form values
        }));
    
        // Call onChange to update the state in the parent component
        onChange(id, "one_opensand_sat_networks", validNetworks);
      }
  ...
  return (
    <div className="bg-gray-100 p-6">
      ...
      <header className="bg-blue-500 text-white text-center p-4 rounded-md shadow-md">
        ...
        <h1 className="text-3xl font-bold">OPENSAND_SAT Configuration</h1>
      ...
      <div className="mt-8 bg-white shadow-md rounded-lg p-6">
        <form>
          ...
          {Object.keys(data).map((key) => {
            ...
            if (key === "one_opensand_sat_networks") {
              return (
